...
namespace WLLibrary.Cache.Memcached
{
	...
	public class MemcachedClient 
	{
        ...
        /// @attention:不要以为KeyExists多有效率，clientlibrary库的实现非常偷懒，
        /// 谨慎使用 add.wolan
		/// </summary>
        ...
        /// <returns></returns>
        public bool KeyExists_Bytes(string key)
        {
            return (Get_Bytes(key, null, true) != null);
        }
        ...
        public bool Set_Bytes(string key, byte[] value)
        {
            return Set_Bytes("set", key, value, DateTime.MaxValue, null);
        }
        ...
        public bool Set(string key, string value, DateTime expiry)
        {
            return Set("set", key, value, expiry, null);
        }

        public bool Set_Bytes(string key, byte[] value, DateTime expiry)
        {
            return Set_Bytes("set", key, value, expiry, null);
        }

        /// <summary>
        /// @brief:将value添加到指定Key的后面
        /// </summary>
        /// <param name="key"></param>
        /// <param name="value"></param>
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool Append(string key, string value, DateTime expiry)
        {
            return Set("append", key, value, expiry, null);
        }

        /// <summary>
        /// @brief:将value添加到指定Key的前面
        /// </summary>
        /// <param name="key"></param>
        /// <param name="value"></param>
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool Prepend(string key, string value, DateTime expiry)
        {
            return Set("prepend", key, value, expiry, null);
        }
		...
		private bool Set(string cmdname, string key, object value, DateTime expiry, object hashCode=null,bool asString=false) 
		{
			...
			try 
			{
                ...
                cmd.Append(cmdname).Append(" ").Append(key).Append(" ").Append(flags).Append(" ")
					.Append(GetExpirationTime(expiry)).Append(" ").Append(length).Append("\r\n");
				...
				if(log.IsInfoEnabled)
				{
					log.Info(GetLocalizedString("set memcached command result").Replace("$$Cmd$$", cmd.ToString()).Replace("$$Line$$", line));
				}
        ...
        private bool Set(string cmdname, string key, string value, DateTime expiry, object hashCode=null)
        {
            if (expiry < DateTime.Now)
                return true;
            ...
            if (value != null)
            {
                ...
                try
                {
                    val = UTF8Encoding.UTF8.GetBytes(value);
                    length = val.Length;
                }
                catch (ArgumentException ex)
                {
        ...
        private bool Set_Bytes(string cmdname, string key, byte[] value, DateTime expiry, object hashCode=null)
        {
            if (expiry < DateTime.Now)
                return true;
        ...
        public bool Cas_Bytes(string key, byte[] value,long cas,DateTime expiry, object hashCode=null)
        {
            if (expiry < DateTime.Now)
                return true;
            ...
            {
                StringBuilder cmd = new StringBuilder(100);
                cmd.Append("cas ").Append(key).Append(" ").Append(flags).Append(" ")
                    .Append(GetExpirationTime(expiry)).Append(" ").Append(length).Append(" ").Append(cas.ToString()).Append("\r\n");
		...
		public object Get(string key,bool valueIsStr) 
		{
            return Get(key, null, valueIsStr);
		}
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public byte[] Get_Bytes(string key)
        {
            return Get_Bytes(key, null, false);
        }
		...
		public object Get(string key, object hashCode, bool asString) 
		{
			...
			try 
			{
                ...
                cmd.Append("get ").Append(key).Append("\r\n");
				if(log.IsDebugEnabled)
				{
					log.Debug(GetLocalizedString("get memcached command").Replace("$$Cmd$$", cmd.ToString()));
				}
        ...
        public byte[] Get_Bytes(string key, object hashCode, bool asString)
        {
            // get SockIO obj using cache key
            SockIO sock = SockIOPool.GetInstance(_poolName).GetSock(key, hashCode);
        ...
        public Dictionary<string,string> Gets(string[] keys, ref Dictionary<string, long> cass, object hashCode = null)
        {

            // get SockIO obj using cache key
            SockIO sock = SockIOPool.GetInstance(_poolName).GetSock(keys[0], hashCode);
            ...
            try
            {
                ...
                cass = new Dictionary<string, long>();
                LoadItems_MultiStr(sock, values,cass);

                if (log.IsDebugEnabled && values!=null)
                {
                    // debug code
                    log.Debug(GetLocalizedString("get memcached result").Replace("$$Results$$", values.Count.ToString(new NumberFormatInfo())));
                }
            ...
            catch (IOException e)
            {
                ...
                if (log.IsErrorEnabled)
                {
                    log.Error(GetLocalizedString("get IOException").Replace("$$Key$$", keys[0]), e);
                }
        ...
        public Dictionary<string,byte[]> Gets_Bytes(string[] keys, ref Dictionary<string,long> cass,object hashCode=null)
        {
            ...
            try
            {
                ...
                // build empty map
                // and fill it from server
                Dictionary<string, byte[]> values = new Dictionary<string, byte[]>(keys.Length);
                cass = new Dictionary<string, long>(keys.Length);
                LoadItems_MultiBytes(sock,values,cass);
        ...
        #region CMEM(Tencent memcached)

        public Dictionary<string, byte[]> Gets_Ext_Bytes(string[] keys, object hashCode, ref Dictionary<string, long> cass,
            ref int error,ref string errorDetail)
        {
            ...
            try
            {
                ...
                Dictionary<string, byte[]> values = new Dictionary<string, byte[]>(keys.Length);
                cass = new Dictionary<string, long>();
                CMEM_Ext_LoadItems_MultiBytes(sock, values,cass,ref error,ref errorDetail);
        ...
        private void CMEM_Ext_LoadItems_MultiBytes(SockIO sock, Dictionary<string, byte[]> values, Dictionary<string, long> cass,
            ref int error, ref string errorDetail)
        {
            error = 0;
            while (true)
            {
                ...
                if (line.StartsWith(VALUE))
                {
                    ...
                    string key = info[1];
                    int result = Convert.ToInt32(info[2]);
                    if (result != 0)
                    {
                        error = result;//只记录非0的异常
                    }
                    int flag = Convert.ToInt32(info[3]);
                    int length = Convert.ToInt32(info[4]);
                    long cas = Convert.ToInt64(info[5]);
                    ...
                    {
                        values = null;
                        errorDetail = Encoding.UTF8.GetString(buf);
                        return;
                    }
                }
                else if (END == line)
        ...
        public Dictionary<string, string> Gets_Ext(string[] keys, object hashCode,ref Dictionary<string,long> cass,
            ref int error, ref string errorDetail)
        {
            ...
            try
            {
                ...
                cass = new Dictionary<string, long>(keys.Length);
                CMEM_Ext_LoadItems(sock, values,cass,ref error, ref errorDetail);
        ...
        /// <param name="values">hashmap to store data into</param>
        /// <param name="asString">if true, and if we are using NativehHandler, return string val</param>
        private void CMEM_Ext_LoadItems(SockIO sock, Dictionary<string, string> values,Dictionary<string,long> cass,
            ref int error, ref string errorDetail)
        {
            error = 0;
            while (true)
            {
                ...
                if (line.StartsWith(VALUE))
                {
                    ...
                    if (error == 0)
                    {
                        ...
                        values[key] = Encoding.GetEncoding(_defaultEncoding).GetString(buf);
                        cass[key] = cas;
                    }
                    else
                    {
		...
		/// <param name="values">hashmap to store data into</param>
		/// <param name="asString">if true, and if we are using NativehHandler, return string val</param>
		private void LoadItems(SockIO sock, Hashtable values, bool asString) 
		{
			while(true) 
			{
				string line = sock.ReadLine();
                if (line.Equals(ERROR))
                {
                    if (log.IsErrorEnabled)
                    {
                        log.Error("LoadItems_MultiBytes ERROR");
                    }
                    return;
                }
				if(log.IsDebugEnabled)
				...
				if(line.StartsWith(VALUE)) 
				{
					string[] info = line.Split(' ');
					string key    = info[1];
					int flag      = Convert.ToInt32(info[2]);
                    int length = Convert.ToInt32(info[3]);
					...
					if((flag & F_SERIALIZED) == 0) 
					{
						if(_primitiveAsString || asString) 
						{
							...
							valueContent = Encoding.GetEncoding(_defaultEncoding).GetString(buf);
						}
						else 
						{
							// decoding object
							try 
							{
								valueContent = NativeHandler.Decode(buf);    
							}
							catch(Exception e) 
							{
								if(log.IsErrorEnabled)
					...
					else 
					{
						...
						try 
						{
							...
							valueContent = new BinaryFormatter().Deserialize(memStream);
							if(log.IsInfoEnabled)
							{
								log.Info(GetLocalizedString("loaditems deserializing").Replace("$$Class$$", valueContent.GetType().Name));
							}
						}
        ...
        private void LoadItems_Bytes(SockIO sock, ref byte[] hm)
        {
            while (true)
            {
                string line = sock.ReadLine();
        ...
        private void LoadItems_MultiStr(SockIO sock, Dictionary<string,string> values,Dictionary<string,long> cass)
        {
            while (true)
            {
                string line = sock.ReadLine();
                ...
                if (line.StartsWith(VALUE))
                {
                    ...
                    int flag = Convert.ToInt32(info[2]);
                    int length = Convert.ToInt32(info[3]);
                    long cas = Convert.ToInt64(info[4]);
        ...
        private void LoadItems_MultiBytes(SockIO sock, Dictionary<string,byte[]> values,Dictionary<string,long> cass)
        {
            while (true)
            {
                string line = sock.ReadLine();
        ...
        private static ResourceManager _resourceManager = new ResourceManager("WLLibrary.StringMessages", typeof(MemcachedClient).Assembly);
